...
- Name: kiosk.spec.js >> Home — Organisation Selection >> clicking Electricity navigates to electricity menu
- Location: tests\kiosk.spec.js:73:3
  ...
  - generic [ref=e3]:
    ...
    - generic [ref=e9]:
      - generic [ref=e11]:
        ...
        - generic [ref=e23] [cursor=pointer]: 3 Jun · 11:11
      - generic [ref=e24]:
        - generic [ref=e25]:
...
> 75  |     await page.waitForLoadState('networkidle');
  ...
  145 | 
  146 | // ── 5. GAS MODULE ────────────────────────────────────────────────────────────
  147 | test.describe('Gas Module', () => {
  148 |   test('gas menu shows all services', async ({ page }) => {
  149 |     await page.goto('/gas-menu');
  150 |     await page.waitForLoadState('networkidle');
  151 |     await page.screenshot(SS('05-gas-menu'));
  152 |     await expect(page.getByText(/New Connection|Connection/i)).toBeVisible();
  153 |     await expect(page.getByText(/Meter|Malfunction/i)).toBeVisible();
  154 |     await expect(page.getByText(/Bill/i)).toBeVisible();
  155 |   });
  156 | 
  157 |   test('gas complaint page has voice input', async ({ page }) => {
  158 |     await page.goto('/gas/complaint');
  159 |     await page.waitForLoadState('networkidle');
  160 |     await page.screenshot(SS('05b-gas-complaint'));
  161 |     await expect(page.getByText(/Gas|Complaint/i)).toBeVisible();
  162 |   });
  163 | 
  164 |   test('gas bills page loads', async ({ page }) => {
  165 |     await page.goto('/gas/bills');
  166 |     await page.waitForLoadState('networkidle');
  167 |     await page.screenshot(SS('05c-gas-bills'));
  168 |     await expect(page.getByText(/Bill/i)).toBeVisible();
  169 |   });
  170 | });
  171 | 
  172 | // ── 6. MUNICIPAL MODULE ──────────────────────────────────────────────────────
  173 | test.describe('Municipal Module', () => {
  174 |   test('municipal menu shows all services', async ({ page }) => {
  175 |     await page.goto('/municipal-menu');
```